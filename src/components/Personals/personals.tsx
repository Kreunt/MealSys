...
export const Personals = () => {
    ...
    const handlePersonalSubmit = () => {
        if (name.length > 0 && age.length > 0 && workingHours.length > 0 && freeDays.length > 0 && area.length > 0 && phone.length > 0) {
    ...
    return (
        <div className="personal-list-wrapper">
            <div className="personal-list-form">
                <div className="form-wrapper" onSubmit={handlePersonalSubmit}>
                    ...
                    <div className="form-row">
                        ...
                        <fieldset>
                            ...
                            <input className="form-input" type="text" id="age" name="age" value={age} onChange={(e) => setAge(e.target.value)} />
                        </fieldset>

                        <fieldset>
                            <label className="form-label" htmlFor="workingHours">Enter working hours:</label>
                            <input className="form-input" type="text" id="workingHours" name="workingHours" value={workingHours} onChange={(e) => setWorkingHours(e.target.value)} />
                        </fieldset>

                        <fieldset>
                            <label className="form-label" htmlFor="freeDays">Enter free days:</label>
                            <input className="form-input" type="text" id="freeDays" name="freeDays" value={freeDays} onChange={(e) => setFreeDays(e.target.value)} />
                        </fieldset>

                        <fieldset>
                            <label className="form-label" htmlFor="area">Enter area:</label>
                            <input className="form-input" type="text" id="area" name="area" value={area} onChange={(e) => setArea(e.target.value)} />
                        </fieldset>
                ...
                <button className="btn btn-add" onClick={handlePersonalSubmit}>Add Personal</button>
            </div>
            <PersonalList personals={personals} loading={loading} handlePersonalRemove={handlePersonalRemove} handlePersonalUpdate={handlePersonalUpdate} />
        </div>
    );
}