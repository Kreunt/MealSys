...
export const Orders = () => {
    ...
    const handleOrderSubmit = () => {
        if (customerId.length > 0 && menuId.length > 0 && date.length > 0 && price.length > 0) {
            handleOrderCreate();
                ...
                <div className="form-wrapper" onSubmit={handleOrderSubmit}>
                    ...
                    <div className="form-row">
                        ...
                        <fieldset>
                            ...
                            <input className="form-input" type="text" id="price" value={price} onChange={e => setPrice(e.target.value)} />
                        </fieldset>
                    </div>
                </div>
                
                <button className="btn btn-add" type="submit" onClick={handleOrderSubmit}>Add Order</button>
            </div>
            <OrdersList orders={orders} loading={loading} handleOrderRemove={handleOrderRemove} handleOrderUpdate={handleOrderUpdate} />
        </div>
    );
}